...
# Definicija GRU neuronske mreže
class GRUModel(nn.Module):
    def __init__(self):
        super(GRUModel, self).__init__()
        self.gru = nn.GRU(input_size=5, hidden_size=32, num_layers=1, batch_first=True)
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.title('Trening i test gubitak kroz epohe - GRU')
plt.legend()
plt.show()
...
plt.ylabel('Cijena (USD)')
plt.title('Stvarne vs Predviđene cijene - GRU')
plt.legend()
plt.show()
...
plt.xlabel('Tjedni (test period)')
plt.ylabel('Volumen')
plt.title('Stvarni vs Predviđeni volumen - GRU')
plt.legend()
plt.show()
...
predictions_df.to_csv('gru_7.csv', index=False)