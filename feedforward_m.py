...
monthly_df['Close_Change'] = monthly_df['Close'].pct_change()
monthly_df['SMA_3'] = monthly_df['Close'].rolling(window=3).mean()
monthly_df['Volatility'] = monthly_df['Close'].rolling(window=3).std()
...
plt.xlabel('Datum')
plt.ylabel('Volumen')
plt.title('Kretanje volumena trgovanja Bitcoina (mjesečno)')
plt.legend()
plt.show()
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.title('Trening i test gubitak kroz epohe - Feedforward NN')
plt.legend()
plt.show()
...
plt.ylabel('Cijena (USD)')
plt.title('Stvarne vs Predviđene cijene - Feedforward NN')
plt.legend()
plt.show()
...
plt.xlabel('Mjeseci (test period)')
plt.ylabel('Volumen')
plt.title('Stvarni vs Predviđeni volumen - Feedforward NN')
plt.legend()
plt.show()
...
predictions_df.to_csv('feedforward_m.csv', index=False)